create table musician (m_no INTEGER NOT NULL PRIMARY KEY,m_name VARCHAR(20),born DATE ,died DATE ,born_in  INTEGER,living_in  INTEGER);

create table place (place_no INTEGER NOT NULL PRIMARY KEY,place_town VARCHAR(20),place_country  VARCHAR(20) );

create table performer (perf_no integer not null primary key,perf_is  integer,instrument varchar(10) not null,perf_type  varchar(10) default 'not known' );
...
create table band (band_no INTEGER NOT NULL PRIMARY KEY,band_name  VARCHAR(20),band_home  INTEGER NOT NULL REFERENCES PLACE (place_no),band_type  VARCHAR(10),b_date DATE ,band_contact INTEGER NOT NULL REFERENCES musician (m_no));
...
create table composition(c_no integer not null primary key,comp_date date, c_title varchar(40) not null,c_in    integer     references place (place_no));
...
create table concert(concert_no integer not null primary key,concert_venue varchar(20),concert_in integer not null  references place (place_no),con_date  date, concert_orgniser integer    references musician (m_no));

create table performance(pfrmnc_no integer not null primary key,gave integer references band (band_no),performed integer references composition (c_no),conducted_by  integer references musician (m_no),performed_in  integer references concert (concert_no));
...
SELECT m_name, instrumentFROM musician JOIN performer ON (m_no = perf_is)JOIN place ON (place_no = born_in)WHERE instrument IN ("guitar", "violin") AND place_country ='England';
